# 后端学习路线

## 工作环境
- [The Missing Semester of Your CS Education](https://missing.csail.mit.edu)

## 数据库
### 入门读物
- [Things You Should Know About Databases](https://architecturenotes.co/things-you-should-know-about-databases/)
- [The Art of PostgreSQL](https://theartofpostgresql.com)
- [Practical SQL for Data Analysis](https://hakibenita.com/sql-for-data-analysis)
- [Next-Level Database Techniques for Developers](https://sqlfordevs.com/ebook)
### 关系型数据库中的范式
- [Normalization of Database](https://www.studytonight.com/dbms/database-normalization.php)
### 数据库索引
- [Indexing in PostgreSQL and applying it to JSONB](https://medium.com/@thegalang/indexing-in-postgresql-and-applying-it-to-jsonb-c99ecf50a443)
- [The Importance of Column Order in PostgreSQL Multicolumn B-tree Indexes](https://medium.com/trendyol-tech/postgresql-multicolumn-b-tree-i̇ndekslerde-column-sırasının-önemi-6ca76043a6c)
- [Re-Introducing Hash Indexes in PostgreSQL](https://hakibenita.com/postgresql-hash-index)
### 其它
- [The FILTER clause in Postgres 9.4](https://medium.com/little-programming-joys/the-filter-clause-in-postgres-9-4-3dd327d3c852)
- [Understanding Recursive Queries in PostgreSQL](https://www.cybertec-postgresql.com/en/recursive-queries-postgresql/)
- [Recursion in SQL Explained Visually](https://medium.com/swlh/recursion-in-sql-explained-graphically-679f6a0f143b)
- [https://hakibenita.com/be-careful-with-cte-in-postgre-sql](https://hakibenita.com/be-careful-with-cte-in-postgre-sql)
- [Understanding Lateral Joins in PostgreSQL](https://www.cybertec-postgresql.com/en/understanding-lateral-joins-in-postgresql/)
- [How to Backup and Restore a PostgreSQL Database](https://www.tecmint.com/backup-and-restore-postgresql-database/)
### 进阶
- [How the PostgreSQL Query Optimizer Works](https://www.cybertec-postgresql.com/en/how-the-postgresql-query-optimizer-works/)
- [Join Strategies and Performance in PostgreSQL](https://www.cybertec-postgresql.com/en/join-strategies-and-performance-in-postgresql/)
- [Understanding How PostgreSQL Executes a Query](https://etutorials.org/SQL/Postgresql/Part+I+General+PostgreSQL+Use/Chapter+4.+Performance/Understanding+How+PostgreSQL+Executes+a+Query/)

## Elixir/Erlang/OTP
### 入门读物
- [Official Guide](https://elixir-lang.org/getting-started/introduction.html)
### 元编程
- [Under the Hood of Metaprogramming](https://blog.appsignal.com/series/under-the-hood-of-metaprogramming)
### 性能
- [Elixir RAM and the Template of Doom](https://www.evanmiller.org/elixir-ram-and-the-template-of-doom.html#notes)
- [Erlang's Tail Recursion is Not a Silver Bullet](https://ferd.ca/erlang-s-tail-recursion-is-not-a-silver-bullet.html)
- [Erlang Efficiency Guide User's Guide](https://www.erlang.org/doc/efficiency_guide/users_guide.html)
### 其他
- [Tips for Improving Your Elixir Configuration](https://felt.com/blog/elixir-configuration)

## 测试
- [How to Write 3v1L, Untestable Code](https://testing.googleblog.com/2008/07/how-to-write-3v1l-untestable-code.html)

## 正则表达式
- [https://stackoverflow.com/questions/1732348/regex-match-open-tags-except-xhtml-self-contained-tags]

## 编码
- [Character Encodings 101](https://notes.burke.libbey.me/character-encodings-101/)
- [Everything you never wanted to know about ANSI escape codes](https://notes.burke.libbey.me/ansi-escape-codes/)

## 杂项
- [How to design better APIs](https://r.bluethl.net/how-to-design-better-apis)
- [Building Webhooks Into Your Application: Guidelines and Best Practices](https://workos.com/blog/building-webhooks-into-your-application-guidelines-and-best-practices)
- [Guidance for Choosing an Elliptic Curve Signature Algorithm in 2022](https://soatok.blog/2022/05/19/guidance-for-choosing-an-elliptic-curve-signature-algorithm-in-2022/)
